...
func (stats PcStatusList) formatTerse() {
	if !nohdrFlag {
		fmt.Println("cache,size,timestamp,mtime,pages,cached,percent")
	}
	for _, pcs := range stats {
...
func FormatStats(stats PcStatusList,flagMap map[string]bool){
	if flagMap["sortFlag"] {  //根据缓存页面desc排序输出
		sort.Slice(stats, func(i, j int) bool {
			return stats[i].Cached > stats[j].Cached
		})
	}
	if flagMap["jsonFlag"] {
		stats.FormatJson(!flagMap["ppsFlag"])
	} else if flagMap["terseFlag"] {
		stats.formatTerse()
	} else if flagMap["histoFlag"] {
		stats.formatHistogram()
	} else if flagMap["unicodeFlag"] {
		stats.formatUnicode()
	} else if flagMap["plainFlag"] {
		stats.formatPlain()
	} else {
		stats.formatText()
	}
